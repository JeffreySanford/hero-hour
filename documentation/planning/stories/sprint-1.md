...
- [ ] Implement Nest endpoint `POST /api/life-profile` and `GET /api/life-profile/:userid`.
- [ ] Add in-memory repository or service `LifeProfileService` (create/update/fetch).
- [ ] Wire Nest module/controller/service for life-profile.
- [ ] Implement Angular `LifeProfileService` API client with actual API URL and error handling.
- [ ] Build LiveProfileComponent form + validation for required fields and role enum.
- [ ] Add life-profile page route as auth-guard-protected.
- [ ] Add Dashboard health call integration for quick status check.
...
- [ ] Add Jest + Nest tests for controller/service/dto
- [ ] Run `nx test api`, `nx test admin-console`, `nx e2e` and fix failures.
